<template>
  <div>
    ...
    <Loading v-if="loading" message="Sending reset code..." />
...
const handleSubmit = async () => {
  if (!email.value.trim()) {
    alert.value = { type: "error", message: "Please enter your email." };
    return;
  }

  loading.value = true;
  try {
    const { data } = await api.post("/user/forgot-password", { email: email.value });
